...
async function getMessages() {
    ...
    await axios.get(`https://mock-api.driven.com.br/api/v4/uol/messages`)
        .then(response => {
            ...
            if (messages.length > 0) {
                ...
                newMessages = newMessages.slice(newMessages.findIndex(message => getIfIsLastSavedMessage(message, newLastMessage)) + 1);
            }

            if (newMessages.length > 0) {
                console.log(`New messages: ${newMessages.length}, messages length: ${document.querySelectorAll('#chat .de-message').length}`);
            }
...
function getIfIsLastSavedMessage(message, newLastMessage) {
    return message.time === newLastMessage.time &&
